...
export class AddEventComponent implements OnInit {
  ...
  //  interestFormArray = this.formBuilder.array([]);
    if(!isChecked){
      this.subinterests = [];
      return;
    }else{
      interestFormArray = this.formBuilder.array([]);
      interestFormArray.push(new FormControl(option));
    }
    switch (option) {
      case "sport":
  ...
  getSelectedInterest(subInterest: string, isChecked: boolean) {
    let interestFormArray = <FormArray> this.eventForm.controls.interests;
    if (isChecked) {
