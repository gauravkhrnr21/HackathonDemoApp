...
export class AddEventComponent implements OnInit {
  ...
  getSelectedInterest(interest: string, isChecked: boolean) {
    ...
    let subInterestFormArray = <FormArray> this.eventForm.controls.subInterests;
    if (isChecked) {
      interestFormArray.controls = [];
      interestFormArray.push(new FormControl(interest));
    } else {
      this.subInterestFormArray.controls = [];
      let index = interestFormArray.controls.findIndex(x => x.value == interest)
      interestFormArray.removeAt(index);
    }
  }

  subInterestFormArray!: FormArray;
  getSelectedSubInterest(subInterest: string, isChecked: boolean) {
    this.subInterestFormArray = <FormArray> this.eventForm.controls.subInterests;
    if (isChecked) {
      this.subInterestFormArray.push(new FormControl(subInterest));
    } else {
      let index = this.subInterestFormArray.controls.findIndex(x => x.value == subInterest)
      this.subInterestFormArray.removeAt(index);
    }
  }
}
