...
export class AddEventComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,private calendar: NgbCalendar, public formatter: NgbDateParserFormatter,private httpClient: HttpClient) {
  ...
  load(){
    this.httpClient.get("http://localhost:8080/events/getEvents").subscribe(event =>{ 
      console.log(event);
      if(event != null){
  ...
  onSubmit(form:FormGroup){
    ...
    if(formData != null){
      this.httpClient.get<any>("http://localhost:8080/events/addEvent").subscribe(event =>{ 
        console.log(event);
        if(event != null){
        //  this.interests = category;
        }
  ...
  getSelectedInterest(interest: string, isChecked: boolean) {
    ...
    if (isChecked) {
     ...
     // subInterestFormArray = this.formBuilder.array([]);
