...
export class AddEventComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,private calendar: NgbCalendar, public formatter: NgbDateParserFormatter,private httpClient: HttpClient,private storageService:TokenStorageService) {
    ...
    this.eventForm = this.formBuilder.group({
      ...
      categortId: ['', [Validators.required, Validators.minLength(4)]],
      subCategoryId: ['', [Validators.required, Validators.minLength(4)]]
    });
  ...
  getInterest(categoryId:number,isChecked: boolean) {
    ...
    if (isChecked) {
      ...
      interestFormArray.push(new FormControl(categoryId));
    } else {
      subInterestFormArray.controls = [];
      let index = interestFormArray.controls.findIndex(x => x.value == categoryId)
  ...
  onSubInterestChange(index:number,subCategoryId: number,isChecked: boolean){
    ...
    if (isChecked) {
      subInterestFormArray.controls = [];
      subInterestFormArray.push(new FormControl(subCategoryId));
    } else {
      subInterestFormArray.controls = [];
      let index = subInterestFormArray.controls.findIndex(x => x.value == subCategoryId)
