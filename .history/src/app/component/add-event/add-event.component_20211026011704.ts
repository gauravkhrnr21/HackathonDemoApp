...
export class AddEventComponent implements OnInit {
  ...
  load(){
    this.httpClient.get<any>(environment.contextpath+"category/getAllCategory").subscribe(categories =>{
  ...
  onSubmit(form:FormGroup){
    ...
    if(formData != null){
      ...
      formData['subCategoryId'] = formData.subCategoryId[0];

      this.httpClient.post<any>(environment.contextpath+"events/addEvent",formData).subscribe(event =>{ 
  ...
  getInterest(categoryId:number,isChecked: boolean) {
    const categoryFormArray = <FormArray> this.eventForm.controls.categoryId;
  ...
  loadSubInterest(index:number,categoryId:number,isChecked: boolean) {
    ...
    this.httpClient.post(environment.contextpath+"subCategory/getAllSubCategoryByCategory",param).subscribe(subCategories =>{ 
