...
export class ManageInterestComponent implements OnInit {
   ...
   // console.log(formData);
    if((formData.interest != null && formData.interest != "") ||(formData.subInterest != null && formData.subInterest != "")){
      if(formData.subInterest !=""){
        this.searchInterest = this.interestData.filter((ele:any) => ele.interest === formData.interest.toLowerCase() && ele.subInterest === formData.subInterest.toLowerCase());
        this.fetchPosts();
      }else{
        this.searchInterest = this.interestData.filter((ele:any) => ele.interest === formData.interest.toLowerCase());
        this.fetchPosts();
      }
  ...
  fetchPosts(): void {
    setTimeout(()=>{
      this.POSTS = this.searchInterest;
    },1000);
  }
