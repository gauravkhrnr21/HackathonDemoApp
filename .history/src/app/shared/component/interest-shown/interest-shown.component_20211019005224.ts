...
export class InterestShownComponent { 
  ...
  onFoodInterest(sport: string, isChecked: boolean) {
    const foodFormArray = <FormArray> this.interestForm.controls.sport;
    if (isChecked) {
      foodFormArray.push(new FormControl(sport));
    } else {
      let index = foodFormArray.controls.findIndex(x => x.value == sport)
      foodFormArray.removeAt(index);
    }
  }

  oncreativityInterest(sport: string, isChecked: boolean) {
    const creativityFormArray = <FormArray> this.interestForm.controls.sport;
    if (isChecked) {
      creativityFormArray.push(new FormControl(sport));
    } else {
      let index = creativityFormArray.controls.findIndex(x => x.value == sport)
      creativityFormArray.removeAt(index);
    }
  }

  technologyInterest(sport: string, isChecked: boolean) {
    const sportFormArray = <FormArray> this.interestForm.controls.sport;
  ...
  stockInterest(sport: string, isChecked: boolean) {
    const sportFormArray = <FormArray> this.interestForm.controls.sport;
    console.log(sportFormArray)
