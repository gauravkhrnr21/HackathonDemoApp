...
export class InterestShownComponent { 
  ...
  parentingInterest(parenting: string, isChecked: boolean) {
    const parentingFormArray = <FormArray> this.interestForm.controls.sport;
    if (isChecked) {
  ...
  shoppingInterest(sport: string, isChecked: boolean) {
    const shoppingFormArray = <FormArray> this.interestForm.controls.sport;

    if (isChecked) {
      shoppingFormArray.push(new FormControl(sport));
    } else {
      let index = shoppingFormArray.controls.findIndex(x => x.value == sport)
