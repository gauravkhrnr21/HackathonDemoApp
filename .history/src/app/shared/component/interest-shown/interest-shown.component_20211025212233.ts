...
export class InterestShownComponent { 
  ...
  load(){
    this.httpClient.get<any>("http://localhost:8080/subCategory/getAllSubCategory").subscribe(resp =>{ 
      this.sports = resp.filter((x:any) => x.groupName === Constants.SPORT);
      this.foods = resp.filter((x:any) => x.groupName === Constants.FOOD);
      this.creativities = resp.filter((x:any) => x.groupName === Constants.CREATIVITY);
      this.technologies = resp.filter((x:any) => x.groupName === Constants.TECHNOLOGY);
      this.stocks = resp.filter((x:any) => x.groupName === Constants.STOCK);
      this.parentings = resp.filter((x:any) => x.groupName === Constants.PARENTING);
      this.shoppings = resp.filter((x:any) => x.groupName === Constants.SHOPPING);
    },error=>{
      console.log(error)
    });
  }
